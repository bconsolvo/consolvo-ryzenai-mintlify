...
- Ensure you have called EP registration before creating the session. See [Execution Providers](/winml/winml_ep).
- Run the application with administrator privileges if the EP requires Microsoft Store download.
- Verify the NPU driver is installed. See [Installation](/winml/installation).
...
- Ensure the model is in a supported format (FP32 or QDQ). See [Model Support](/winml/model_support).
- For quantized models, verify the quantization configuration (A8W8 for CNN, A16W8 for Transformer).
- Check model opset compatibility. ONNX opset 17 is recommended. See [Model compilation and deployment](/modelrun).
...
- Set execution policy to `PREFER_NPU` or explicitly use `VitisAIExecutionProvider`. See [Execution Providers](/winml/winml_ep).
- Verify the NPU driver is installed and the device is recognized.
- Check that the model is compatible with the Vitis AI EP. See [Model Support](/winml/model_support).
...
**Solution:** Run pywinrt-related code in a **separate process**. Use the subprocess pattern shown in [Execution Providers](/winml/winml_ep) (place `winml.py` in the same directory as your application script).
