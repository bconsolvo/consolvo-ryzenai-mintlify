...
description: "Use the xrt-smi command-line utility to monitor, validate, and configure the AMD NPU."
---
...
<Note>
The `examine --report aie-partition` report runtime information. These commands should be used when a model is running on the NPU. You can run these commands in a loop to see live updates of the reported data.
</Note>
...
<Note>
Power reporting is not supported on PHX and HPT NPUs. Power reporting is only available on STX devices and onwards. Report "Estimated Power" is currently unavailable for Linux users.
</Note>
...
<Note>
Some sanity checks may fail if other applications (for example MEP, Microsoft Experience Package) are also using the NPU.
</Note>
