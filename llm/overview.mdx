...
- **Standalone Windows GUI installer**: Quick setup with a desktop shortcut for immediate use. (Recommended for end users, see [Server Interface](/llm/server_interface))
- **Full Lemonade SDK**: Complete development toolkit with server interface included. (Recommended for developers, see [High-Level Python SDK](/llm/high_level_python))
...
To get started in Python, follow these instructions: [High-Level Python SDK](/llm/high_level_python).
...
To get started with the OGA APIs, follow these instructions: [Hybrid OGA](/hybrid_oga).
...
For the collection of supported models, visit [Pre-Optimized Models](/llm_list#pre_opt_model).

It is also possible to run fine-tuned versions of the models listed (for example, fine-tuned versions of Llama2 or Llama3). For instructions on how to prepare a fine-tuned OGA model, refer to [Preparing OGA Models](/oga_model_prepare).
