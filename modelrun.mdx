...
For a complete list of supported operators, consult this page: [Supported Operators](/ops_support).
...
<Note>
- Running BF16 Models is only supported for STX/KRK or newer devices. For the model compatibility table see [Release Notes](/relnotes).
- For C++ applications that need to compile BF16 models at runtime, include `${CONDA_PREFIX}/Lib/site-packages/flexml/flexml_extras/lib/vaiml.dll` along with the DLLs specified in [Application Development](/app_development#app-development). However, using pre-compiled BF16 models is recommended for C++ deployment.
</Note>
...
Suitable settings for the `target` and `xclbin` provider options are dependent on the type of device. The application must perform a device detection check before configuring the Vitis AI EP. For more details on how to do this, refer to the [Application Development](/app_development) page.
