---
title: "Compiling Operators for OGA Models"
description: "Experimental recipe for compiling operators when a fine-tuned model introduces new operator shapes."
---
...
When users fine-tune these models, only the weights change and no new operator shapes are introduced. In that case, follow the steps from [Preparing OGA Models](/oga_model_prepare) to prepare the model, which will run on the Ryzen AI runtime using the precompiled operators.
...
1. Ensure the model is quantized following the [quantization recipe](/oga_model_prepare#quantization).
...
Follow the [official guide](/hybrid_oga) to copy `model_benchmark.exe` and required DLL dependencies to the current working directory. Then run:
