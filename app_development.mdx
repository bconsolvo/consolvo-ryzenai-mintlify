...
For INT8 models, the application should detect the type of APU present (PHX, HPT, STX, or KRK) and set the `target` and `xclbin` provider options accordingly. Refer to the section on [using INT8 models](/modelrun#int8-models) for more details.

For BF16 models, the application should set the `config_file` provider option to the same file that was used to precompile the BF16 model. Refer to the section on [using BF16 models](/modelrun#bf16-models) for more details.
...
AMD recommends using the ONNXRuntime [EP Context Cache](/modelrun#ort-ep-context-cache) feature for saving and reloading compiled models.
...
- Recommended but not mandatory: pre-compiled models in the form of [Onnx Runtime EP context models](/modelrun#ort-ep-context-cache)
...
- Pre-compiled models in the form of [Vitis AI EP cache folders](/modelrun#vitisai-ep-cache)
